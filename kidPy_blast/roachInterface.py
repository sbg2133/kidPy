...
class roachInterface(object):
    ...
    def __init__(self, index, fpga, gc, regs):
        self.gc = gc
        self.fpga = fpga
        self.regs = regs
    ...
    def plotPFB(self):
        fig = plt.figure()
        plot1 = fig.add_subplot(111)
        line1, = plot1.plot(np.arange(2**15), np.zeros(2**15), label = 'I', color = '#FF4500')
        line2, = plot1.plot(np.arange(2**15), np.zeros(2**15), label = 'Q', color = 'purple')
        plt.grid()
        plt.tight_layout()
        plt.ylim(-0.70, 0.70)
        count = 0 
        stop = 1.0e6
        while(count < stop):
            self.fpga.write_int('PFB_pfb_pfb_snap_ctrl',0)
            self.fpga.write_int('PFB_pfb_pfb_snap_ctrl',1)
            self.fpga.write_int('PFB_pfb_trig',0)
            self.fpga.write_int('PFB_pfb_trig',1)
            self.fpga.write_int('PFB_pfb_trig',0)
            snap = (np.fromstring(self.fpga.read('PFB_pfb_pfb_snap_bram', (2**14)*8), dtype='>i2')).astype('float')
            I0 = snap[0::4]
            Q0 = snap[1::4]
            I1 = snap[2::4]
            Q1 = snap[3::4]
