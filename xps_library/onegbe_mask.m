...
for i =1:length(gateway_outs)
    ...
    elseif regexp(get_param(gw,'Name'),'_tx_srcip$')
        set_param(gw,'Name',clear_name([cursys,'_tx_srcip']));
    elseif regexp(get_param(gw,'Name'),'_tx_srcmac$')
        set_param(gw,'Name',clear_name([cursys,'_tx_srcmac']));
    elseif regexp(get_param(gw,'Name'),'_tx_destmac$')
        set_param(gw,'Name',clear_name([cursys,'_tx_destmac']));
    elseif regexp(get_param(gw,'Name'),'_tx_srcport$')
        set_param(gw,'Name',clear_name([cursys,'_tx_srcport']));
    else
        errordlg(['Unkown gateway: ',get_param(gw,'Parent'),'/',get_param(gw,'Name')]);
    end
end
