 ...
 * (USB_ENDPOINT_IN).
 */
hid_return hid_interrupt_read(HIDInterface *  hidif,
     unsigned int ep, char*  bytes, unsigned int  size, 
 ...
 */
hid_return hid_interrupt_write(HIDInterface *  hidif,
     unsigned int  ep, const char*  bytes, unsigned int  size, 
     unsigned int  timeout) 
{
  ASSERT(hid_is_initialised());
  ASSERT(hid_is_opened(hidif));
